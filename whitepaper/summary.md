...
道易程是基于以太坊的一个通过优良的治理（七大治理协议和一大治理网络 IADD Network）让DAO（1.0和2.0）繁衍生息、蓬勃发展的平台。

只需几分钟时间，任何人都可以在道易程上创建一个去中心化自治智能组织，并且，这家DAO已经可以开始运作了！

是的，道易程为DAO的创建和运营准备了非常独特的基础：道易程框架和完全开放的dApp市场。

在道易程框架中，它内置的多种协议和通用的 IADD 网络将在DAO代币的市场供应、定价通证的锻造、去中心化交易、去中心化支付、去中心化基金会等方面提供统一的应用标准、技术标准和全局功能，实际上道易程实践的是以上所有方面的有效治理。而说到治理，特别值得一提的是具备技术标准和应用标准双重创新的 IADD 网络在去中心化交易和去中心化支付方面做到了合二为一，并且完全无人值守。
...
（深圳大学区块链课程《区块链概论》，[https://u.naturaldao.io/be/](https://u.naturaldao.io/be/)）
...
_你可以把最近出现的很多人类协作的DAO（1.0的DAO）视为大家正在摸索区块链协作治理的链下部分，而我们这里讨论的 DAO 2.0 则是**至少**其链上的核心得到通用的有效治理的DAO。**简而言之：道易程是以太坊公链之上的一个治理层，而其链上通证经济行为接受道易程治理（良好的链上治理）的DAO就是DAO 2.0。**_

_**DAO 2.0 意味着这类DAO的核心都可以描绘为：**_

1. DAO = 共识 + 通证 + 智能合约 + 智能算法\
   我们已经知道通证和智能合约是区块链的两大硬核。而对于DAO来说，我们认为每个DAO还必须智能化，才能完成自我的治理——或者说，我们要通过引入智能算法，使得DAO能够摆脱人的不良干扰。
2. 遵循治理最小化及可信的中立性这两条基本准则\
   所以DAO的核心都必须根植于一个在这两条原则的指导下的通用的治理层。
3. 未来几乎没有哪个DAO是孤立的。每个DAO都能以其智能响应其它DAO的交互请求，每个DAO也能基于其智能向其它DAO发出交互请求，并且消化（吸收营养、摒弃垃圾）所获取的数据或其它反馈信息。因此，每个DAO的经济模型（如dApp里面设置的收费标准）就不能朝三暮四，随意变化，以保持对用户和对外部合约的友好和稳定性。
4. 一个DAO可能有自己的自然寿命，也可能会非正常死亡（如被黑、被不良DAO毒害），此时也会有新的DAO接替它。

AI 的发展正扑面而来，它与区块链的强强联合也只是时间问题。我们建议将2.0的 DAO 提升为Decentralized Autonomous Organism。吐故纳新，是为了保证DAO还能与时俱进，不断防范人类bug带来的破坏。
...
现在有数十个团队，在以太坊客户端、以太坊网络的扩展层（如Layer2）、EIPs（以太坊改进提案）三个层面进行协作，以完成以太坊2.0的宏图大业。这将使得ETH2.0完成之时，其工程量可领先于单个公链数十年之久。另外，以太坊2017年引爆区块链，现在以太坊上已经有成百上千个DApp，无论是项目总数、通证总量和活跃度，都超越其它大多数公链十年至数十年。由此可知，以太坊2.0很有可能在公链竞争中脱颖而出，所以我们目前以太坊作为道易程的基础。我们也已经将解决一切通证的链上去中心化存储方案总结为[**EIP-2569**](https://github.com/ethereum/EIPs/pull/2569)，以及我们创立的新的通证标准[EIP-3712](https://github.com/naturaldao/EIPs/blob/Eip-Branch/EIPS/eip-3712.md)，贡献给了以太坊社区。
