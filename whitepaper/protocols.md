...
同時，該協議中還包含了一個智能公器許可證，其名稱為 SC0（意為Smart Commons 0）。即Minting操作，也就是宣布某個智能合約或某個dApp（某些智能合約的組合）自身成為以太坊的公共資源（Public Goods）——我們稱其為智能公器（Smart Common）。這也就是說，接受道易程治理的dApp，即為智能公器。
